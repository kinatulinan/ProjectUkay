...
public class PaymentController {
    ...
    @GetMapping(value = "/showPayment/{user_id}")
    ...
    @PostMapping(value = "/savePayment/{user_id}")
    public ResponseEntity<String> savePayment(@PathVariable int user_id, @RequestBody PaymentEntity payment) {
        Optional<UserEntity> userOptional = uRepo.findById(user_id);
        if (userOptional.isPresent()) {
            try {
                payment.setUser(userOptional.get());
                pRepo.save(payment);
                return ResponseEntity.ok("Payment saved with user ID: " + user_id);
            } catch (Exception e) {
            ...
            return ResponseEntity.badRequest().body("User not found with ID: " + user_id);
        }
    }


    @PutMapping(value = "/updatePayment/{paymentid}/{user_id}")
    public ResponseEntity<String> updatePayment(@PathVariable int paymentid, @PathVariable int user_id, @RequestBody PaymentEntity paymentdets) {
        Optional<PaymentEntity> optionalPayment = pRepo.findById(paymentid);
        Optional<UserEntity> optionalUser = uRepo.findById(user_id);
